...
local function compareUpvalue(query, upvalue, ignore)
	...
	local stringCheck = upvalueType == "string" and (query == upvalue or string.match(string.lower(upvalue), string.lower(query)))
	local numberCheck = not ignore and upvalueType == "number" and not isTableIndex and (tonumber(query) == upvalue or string.format("%.2f", upvalue) == query)
	...
	elseif upvalueType == "function" then
		...
		return query == closureName or string.match(string.lower(closureName), string.lower(query))
	end
