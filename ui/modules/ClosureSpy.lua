...
function Log.new(hook)
	...
	local function viewLogs()
		...
		for _, call in pairs(hook.Logs) do
...
ignoreContextSelected:SetCallback(function()
	for _, log in pairs(selected.logs) do
		local hook = log.Hook
...
removeConditionContextSelected:SetCallback(function()
	for _, condition in pairs(selected.conditions) do
		condition:Remove()
	end
...
conditionStatus:SetCallback(function(_, selected)
...
conditionType:SetCallback(function(_, selected)
...
conditionValueType:SetCallback(function(_, selected)
