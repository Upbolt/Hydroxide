...
local function addConstants()
	...
	if string.gsub(query, " ", "") ~= "" then
		if not tonumber(query) and string.len(query) <= 1 then
			return
		end
		...
		for _, closure in pairs(Methods.Scan(query)) do
			Log.new(closure)
		end
...
viewConstantsContext:SetCallback(function()
	if selectedLog then
		...
		if temporaryConstants then
			for _, constantLog in pairs(temporaryConstants) do
